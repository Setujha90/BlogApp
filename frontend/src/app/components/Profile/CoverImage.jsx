...
const CoverImage = ({coverImage, id}) => {
  ...
  const [editCoverImage, setEditCoverImage] = useState("https://images.pexels.com/photos/268941/pexels-photo-268941.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1")

  return (
    <div className='relative'>
      <Image onClick={(e) => {setEditImage(!editImage)}} className={`${editImage && "blur-sm"} w-[100%] h-[200px] object-cover object-center`}
        width={800}
        height={50}
        ...
        <AuthLoggedInUser userId={id}>
          <div className={`${!editImage && "hidden"} flex gap-3 items-center text-xl absolute top-[50%] left-[50%] -translate-y-[50%] -translate-x-[50%]`}>
            <input onChange={(e) => {
              ...
              setUploadDone(true)
            }} className='hidden' type="file" name="coverImage" id="coverImage" />
